#!/usr/bin/env python3
"""
Generate TVA FSDS forcing data (1980-1999, 20 years, 3-hour resolution)
Corresponding to crujra.v2.5.5d_FSDS_1901-2023_z01.nc
...
start_year = 1980
end_year = 1999
final_output_file = os.path.join(out_dir, f"TVA_FSDS_{start_year}-{end_year}.nc")

print("="*80)
print("Generate TVA FSDS forcing data")
...
for year in range(start_year, end_year + 1):
    for month in range(1, 13):
        file_name = f"clmforc.Daymet.km.1d.Solr.{year}-{month:02d}.nc"
...
try:
    with xr.open_mfdataset(all_monthly_files, combine='nested', concat_dim='time',
        ...
        # === Process FSDS variable ===
        if 'FSDS' not in ds.data_vars:
            print("Error: FSDS variable not found.")
            sys.exit(1)
        
        print("Processing FSDS variable...")
        ds_temporal = ds[['time', 'FSDS']]
        ...
        output_encoding = {
            'FSDS': {'zlib': True, 'complevel': 4, 'dtype': 'float32'},
