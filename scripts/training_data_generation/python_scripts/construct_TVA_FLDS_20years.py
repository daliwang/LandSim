#!/usr/bin/env python3
"""
Generate TVA FLDS forcing data (1980-1999, 20 years, 3-hour resolution)
Corresponding to crujra.v2.5.5d_FLDS_1901-2023_z01.nc
...
start_year = 1980
end_year = 1999
final_output_file = os.path.join(out_dir, f"TVA_FLDS_{start_year}-{end_year}.nc")

print("="*80)
print("Generate TVA FLDS forcing data")
...
for year in range(start_year, end_year + 1):
    for month in range(1, 13):
        file_name = f"clmforc.Daymet.km.1d.TPQWL.{year}-{month:02d}.nc"
...
try:
    with xr.open_mfdataset(
        ...
        # === Process FLDS variable ===
        time_varying_vars = ['FLDS']
        if 'FLDS' not in ds.data_vars:
            print("Error: FLDS variable not found.")
            sys.exit(1)
        ...
        for var_name, data_array in static_data.items():
            ds_corrected_time[var_name] = data_array
        ...
        output_encoding = {
            'FLDS': {'zlib': True, 'complevel': 4, 'dtype': 'float32'},
