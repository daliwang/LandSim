#!/usr/bin/env python3
"""
Generate TVA QBOT forcing data (1980-1999, 20 years, 3-hour resolution)
Corresponding to crujra.v2.5.5d_QBOT_1901-2023_z01.nc
...
start_year = 1980
end_year = 1999
final_output_file = os.path.join(out_dir, f"TVA_QBOT_{start_year}-{end_year}.nc")

print("="*80)
print("Generate TVA QBOT forcing data")
...
try:
    with xr.open_mfdataset(all_monthly_files, combine='nested', concat_dim='time',
        ...
        # === Process QBOT variable ===
        if 'QBOT' not in ds.data_vars:
            print("Error: QBOT variable not found.")
            sys.exit(1)
        
        print("Processing QBOT variable...")
        ds_temporal = ds[['time', 'QBOT']]
        ...
        output_encoding = {
            'QBOT': {'zlib': True, 'complevel': 4, 'dtype': 'float32'},
