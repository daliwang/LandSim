#!/usr/bin/env python3
"""
Generate TVA TBOT forcing data (1980-1999, 20 years, 3-hour resolution)
Corresponding to crujra.v2.5.5d_TBOT_1901-2023_z01.nc
...
start_year = 1980
end_year = 1999
final_output_file = os.path.join(out_dir, f"TVA_TBOT_{start_year}-{end_year}.nc")

print("="*80)
print("Generate TVA TBOT forcing data")
...
try:
    with xr.open_mfdataset(all_monthly_files, combine='nested', concat_dim='time',
        ...
        # === Process TBOT variable ===
        if 'TBOT' not in ds.data_vars:
            print("Error: TBOT variable not found.")
            sys.exit(1)
        
        print("Processing TBOT variable...")
        ds_temporal = ds[['time', 'TBOT']]
        ...
        output_encoding = {
            'TBOT': {'zlib': True, 'complevel': 4, 'dtype': 'float32'},
