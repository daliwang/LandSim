...
| File | Purpose |
|------|---------|
| `CNP_IO_e3smv3_h0.txt` | Variable list; points `TRENDY1_PATH` to E3SMv3_h0 only |
...
# 3.1 Full-grid inference (longest step)
python scripts/run_inference_all.py \
  --model "$PHASE2_RUN_DIR/cnp_model.pt" \
...
- **Training time:** E3SMv3_h0 has ~396 batches vs ~20k samples in the Trendy Phase 1 run; expect significantly longer training per epoch. Use `nohup` or a job scheduler.
...
- **Inference:** Scripts use `--no-derive-np-from-c` (recommended; see `docs/RERUN_PHASE2_PHASE3_NO_CNP_FIX.md`). Override with `export INFERENCE_EXTRA_FLAGS=""` if you want the legacy behavior.
